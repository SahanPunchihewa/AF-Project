    ...
    return ( 
        ...
        <div className="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
					<table className="ml-32 w-[1000px] border-collapse bg-white text-left text-sm text-gray-500">
						...
						}).filter((elem) => elem.complaintStatus == "invalid" && elem.citizenId._id===id)
						.map((complaint, key) => (
							<tbody className="divide-y divide-gray-100 border-t border-gray-100 h-24" key={key}>
								<tr className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="w-24 py-2 bg-red-200 ml-1 border-2 border-red-600 rounded"> 
                                        <div className="ml-5 font-semibold text-red-600 ">{complaint.complaintStatus}</div>
                                        </div>
                                    </td>
									<td className="px-6 py-4">
										<div>
		...
		<div className="font font-semibold text-2xl ml-[480px] mb-10" >No Invalid Complaints</div>
	</div>)}
        </>
     );
}
 
export default InvalidComplaints;